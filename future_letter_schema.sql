-- 未来情书功能数据库表设计
-- Future Letter Feature Database Schema
...
CREATE TABLE future_letter (
    ...
    -- 接收方式（三种方式二选一）
    delivery_method ENUM('PARTNER', 'PHONE', 'WECHAT') NOT NULL DEFAULT 'PARTNER' COMMENT '发送方式：情侣对方/手机号/微信号',
    receiver_phone VARCHAR(20) COMMENT '接收者手机号（如选择手机号方式）',
    receiver_wechat VARCHAR(100) COMMENT '接收者微信号（如选择微信号方式，可选）',
